...
\begin{itemize}
    \item Cointegration window of 365 days
    \item We create a set of cointegration windows, starting from the 2015-01-01 of the dataset, and rolling 182 days at a time (i.e. 6 months) until the end of the dataset at 2024-12-31.
    \item Inside each cointegration window, we apply the following steps to create entire set of inputs for the model:
    \begin{itemize}
        \item We calculate the log cumulative returns of all the assets, using Equation~\ref{eq:log_cumulative_return}.
        \item We apply cointegration to the log returns of the assets, to obtain the time-series residuals for each asset, using Equation~\ref{eq:cointegration_all}.
        \item We create a set of cumalative residual windows of 30 days starting from the first day of the cointegration period, and rolling one day at a time, until the end of the cointegration period. This gives us a set of cumulative residuals for each asset, using Equation~\ref{eq:cumulative_residual}.
        \item For each cumuatlive residual window, we find the next day return for each asset, using Equation~\ref{eq:log_cumulative_return}, where $P_{i,0}$ is the price of asset $i$ at the start of the cointegration period, and $P_{i,t+1}$ is the price of asset $i$ one day after the end of the cumulative residual window. 
        \item For each cumulative residual window, we also gather the weather data for the same period, and concatenate it with the cumualative residuals to create a input for the model.
    \end{itemize}
    \item The output of the model is a vector of portfolio weights for each asset, which is soft normalized to sum to 1.
    \item We multiply the portfolio weights by the returns of the assets one day after the end of the cumulative residual window, to get the next day return for that input.
    \item We get a set of next returns, the size of which is determined by the batch size during training.
    \item We calculate the Sharpe ratio of the set of next day returns, and use the negative of the Sharpe ratio as the loss function.
\end{itemize}

\section*{Input Preparation Pipeline}

To train the deep learning model for statistical arbitrage, we follow these steps:

\subsection*{Cointegration Windowing}
\begin{itemize}
...
\subsection*{Within each cointegration window:}

\begin{enumerate}
    \item \textbf{Log Cumulative Returns}:
    \begin{itemize}
        \item For each asset $i$, calculate the log cumulative returns as defined in Equation~\eqref{eq:log_cumulative_return}.
        \item Shapes:
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item For each starting index $t_i$, we compute a sequence of cumulative sums over the next 30 days, as defined in Equation~\eqref{eq:cumres}:
    ...
    \begin{itemize}
        ...
        \]
        where $\mathbf{R}_{t+1}$ is defined by Equation~\eqref{eq:logreturn}.
    \end{itemize}
\end{enumerate}
